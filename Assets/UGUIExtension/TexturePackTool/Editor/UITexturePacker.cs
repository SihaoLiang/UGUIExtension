...
public class UITexturePacker
{
            ...
            for (xPadding = 1; xPadding >= 0; --xPadding)
            {
                for (yPadding = 1; yPadding >= 0; --yPadding)
                {
                    ...
                    else if (xPadding == 0 && yPadding == 0)
                    {
                        ...
                        bp.ExpandTextureSize(width *= (width <= height ? 2 : 1), height *= (height < width ? 2 : 1));
                        bExpand = true;
                    }
                }
    ...
    public void ExpandTextureSize(int width, int height)
    {
        if (width < binHeight || height < binHeight)
            return;
        ...
        for (int i = 0; i < freeRectangles.Count; i++)
        {
            ...
            rect.width = freeRectangles[i].width  +  width - binWidth;
            rect.height = freeRectangles[i].height + height - binHeight;
    ...
    void MergeFreeList()
    {
        for (int i = 0; i < freeRectangles.Count; ++i)
            for (int j = i + 1; j < freeRectangles.Count; ++j)
            {
                if (IsMerge(freeRectangles[i], freeRectangles[j]))
                {
                    Rect a = freeRectangles[i];
                    Rect b = freeRectangles[j];
    ...
    bool IsMerge(Rect a, Rect b)
    {
        ...
        float area = a.width * a.height + b.width * b.height;

        return (int)(area * 1000) == (int)((xMax - xMin) * (yMax - yMin) * 1000);
    }
}
